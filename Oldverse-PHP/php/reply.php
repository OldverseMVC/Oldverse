...
$stmt->execute();
if($stmt->error){
    showError(500, "An error occured while trying to fetch the comment from the database.");
}
$result = $stmt->get_result();
if($result->num_rows==0){
    showError(404, "The comment could not be found.");
}
      ...
      <form method="post" class="edit-post-form">
        ...
        <select name="edit-type">
          <option value selected>Select an option.</option>
            <option value="spoiler" data-action="/replies/<?= $_GET['id'] ?>.set_spoiler.json">Set as spoiler</option>
          <option value="delete" data-action="/replies/<?= $_GET['id'] ?>.delete.json">Delete</option>
        </select>
         <div class="form-buttons">
          <input type="button" class="olv-modal-close-button gray-button" value="Cancel">
